...
export class EscalationService {
  ...
  constructor(
    ...
    // GoalDecompositionService가 제공되지 않으면 경고만 출력
    if (!goalDecompositionService) {
      this.logger.warn(
        'GoalDecompositionService not available - automatic task decomposition disabled',
      );
    }
  }
  ...
  async decomposeTask(request: EscalationRequest): Promise<EscalationResult> {
    ...
    if (this.goalDecompositionService) {
      try {
        ...
        this.logger.error(
          `Automatic decomposition failed: ${err.message}`,
          err.stack,
        );
    ...
    this.logger.log(
      `Task ${request.taskId} marked for manual decomposition. ` +
  ...
  /**
   * Task를 임시 Goal로 변환 (GoalDecomposition을 위해)
   */
  private async convertTaskToGoal(task: Task): Promise<Goal> {
    const dataSource = this.taskRepo.manager.connection;
    const goalRepo = dataSource.getRepository(Goal);

    // 임시 Goal 생성 - 필수 필드만 설정
    const tempGoal = goalRepo.create({
      organizationId: task.organizationId,
      title: `[Auto-decompose] ${task.title}`,
      description: task.description || task.title,
      status: 'active' as any, // GoalStatus.ACTIVE
      goalType: 'objective' as any, // GoalType.OBJECTIVE
      priority: 'medium' as any, // GoalPriority.MEDIUM
      metricType: 'binary' as any, // MetricType.BINARY
      targetValue: 1,
      autoGenerated: true,
      autoDecomposed: true,
    });

    const saved = await goalRepo.save(tempGoal);
    return saved as Goal;
  }

  /**
   * 분해된 서브태스크를 원본 Task에 연결
   */
  private async linkSubtasksToParent(
    parentTaskId: string,
    decomposition: any,
  ): Promise<void> {
    // decomposition.tasks에서 생성된 Task ID들을 가져와서
    // parentTaskId를 설정
    if (decomposition.tasks && decomposition.tasks.length > 0) {
      for (const subtaskId of decomposition.tasks) {
        await this.taskRepo.update(subtaskId, {
          parentTaskId: parentTaskId,
        });
      }

      this.logger.log(
        `Linked ${decomposition.tasks.length} subtasks to parent ${parentTaskId}`,
      );
    }
  }
