        ...
        [
          {
            ...
            patterns: [/\+?[\d\s()-]{10,}/g, /\(\d{3}\)\s?\d{3}-\d{4}/g],
            confidence: 0.85,
            priority: 90,
          },
        ],
        ...
        HollonEntityType.TECHNOLOGY,
        {
          entities: new Map([
            [
              'typescript',
              {
                type: HollonEntityType.TECHNOLOGY,
                value: 'TypeScript',
                category: 'language',
              },
            ],
      ...
      [
        HollonEntityType.PROGRAMMING_LANGUAGE,
        {
          entities: new Map([
            [
              'python',
              { type: HollonEntityType.PROGRAMMING_LANGUAGE, value: 'Python' },
            ],
            [
              'javascript',
              {
                type: HollonEntityType.PROGRAMMING_LANGUAGE,
                value: 'JavaScript',
              },
            ],
            [
            ...
            [
              'java',
              { type: HollonEntityType.PROGRAMMING_LANGUAGE, value: 'Java' },
            ],
            // ... more languages
          ]),
          caseSensitive: false,
  ...
  private aggregateByWeightedScore(entities: Entity[]): Entity[] {
    ...
    for (const [, duplicates] of grouped) {
      ...
      for (const entity of duplicates) {
        const weight =
          weights[entity.extractionMethod as keyof typeof weights] || 1.0;
...
class DependencyParsingRelationshipExtractor {
  ...
  private findMatchingEntity(token: string, entities: Entity[]): Entity | null {
    return (
      entities.find(
        (e) =>
          e.value.toLowerCase().includes(token.toLowerCase()) ||
...
class SVMRelationshipClassifier {
  ...
  extractRelationshipFeatures(
    ...
    const sourceTokenIndex = this.findTokenIndex(
      source.value,
      parseResult.tokens,
    );
    const targetTokenIndex = this.findTokenIndex(
      target.value,
      parseResult.tokens,
    );
    ...
    return {
      ...
      deprelsBetween: this.getDependencyRelations(
        startIdx,
        endIdx,
        parseResult,
      ),

      lowestCommonAncestor: this.findLCA(
        sourceTokenIndex,
        targetTokenIndex,
        parseResult,
      ),
      shortestPath: this.findShortestPath(
        sourceTokenIndex,
        targetTokenIndex,
        parseResult,
      ),
      ...
      distanceToSentenceEnd:
        parseResult.tokens.length -
        Math.max(sourceTokenIndex, targetTokenIndex),
    };
  }

  classify(features: RelationshipClassificationFeatures): {
    type: string;
    confidence: number;
  } {
  ...
  private featuresToVector(
    features: RelationshipClassificationFeatures,
  ): number[] {
...
class DistantSupervisionRelationshipExtractor {
  ...
  private checkKnowledgeBase(entity1: Entity, entity2: Entity): string | null {
...
class TransformerRelationshipExtractor {
  ...
  private markEntities(text: string, entity1: Entity, entity2: Entity): string {
    ...
    const sorted =
      entity1.start < entity2.start ? [entity1, entity2] : [entity2, entity1];

    // Replace in reverse order to maintain indices
    marked =
      marked.substring(0, sorted[1].start) +
      '[E2] ' +
      text.substring(sorted[1].start, sorted[1].end) +
      ' [/E2] ' +
      marked.substring(sorted[1].end);

    marked =
      marked.substring(0, sorted[0].start) +
      '[E1] ' +
      text.substring(sorted[0].start, sorted[0].end) +
      ' [/E1] ' +
...
class HybridRelationshipExtractor {
  ...
  async extract(
    ...
    const results = await Promise.all([
      ...
      useDeepLearning
        ? this.transformer.extract(text, entities)
        : Promise.resolve([]),
    ]);
  ...
  private mergeRelationships(relationships: Relationship[]): Relationship[] {
    ...
    for (const [, rels] of grouped) {
      ...
      const avgConfidence =
        rels.reduce((sum, r) => sum + r.confidence, 0) / rels.length;
...
class UniversalConversationProcessor {
  ...
  async processConversation(
    ...
    if (!format.validator.validate(data)) {
      throw new Error(
        `Validation failed: ${format.validator.getErrors().join(', ')}`,
      );
    }
...
class SlackConversationAdapter {
  ...
  private adaptMessage(slackMsg: SlackMessage): UnifiedMessage {
    return {
      ...
      media:
        slackMsg.blocks?.map((b) => ({
          type: 'block',
          data: b,
        })) || [],
      reactions:
        slackMsg.reactions?.map((r) => ({
          emoji: r.name,
          count: r.count,
        })) || [],
  ...
  adapt(emailThread: EmailThread): UnifiedConversation {
    return {
      ...
      messages: emailThread.messages.map((msg) =>
        this.adaptMessage(msg, emailThread),
      ),
      metadata: {
...
class ExtractionValidator {
  ...
  validateRelationship(
    ...
    const sourceExists = entities.some(
      (e) =>
        e.value === relationship.source.value &&
        e.type === relationship.source.type,
    );
    ...
    const targetExists = entities.some(
      (e) =>
        e.value === relationship.target.value &&
        e.type === relationship.target.type,
    );
